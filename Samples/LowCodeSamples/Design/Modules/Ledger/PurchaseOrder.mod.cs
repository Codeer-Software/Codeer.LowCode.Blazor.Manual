...
void DetailLayoutDesign_OnAfterInitialization()
{
    ...
    row.数量.Value = 5;
    row.単価.Value = 1980;
    row.発注日.Value = orderDate.ToString();
    row.納期.Value = deliveryDate.ToString();
...
void 発注書明細リスト_OnDataChanged()
{
    if(firstLoad)return;
    using var suspend = SuspendNotifyStateChanged();
    合計金額.Value = 0;
    foreach(var i in 発注書明細リスト.Rows)
    {
        合計金額.Value += i.合計.Value;
    }
}

void PDFボタン_OnClick()
{
    出力日.Value = DateTime.Now.ToString();
