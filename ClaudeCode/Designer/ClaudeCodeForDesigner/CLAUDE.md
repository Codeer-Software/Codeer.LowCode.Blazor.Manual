...
各パターンの末尾には **「標準パターン集の対応」** セクションがあり、`Source/TestData/PatternShowcase/` (または `PatternShowcaseAuth/`) のどのモジュールが実装サンプルかを示している。例: 「マスタ参照 (多対1)」パターン → `Modules/Shop.mod.json` + `Modules/ShopType.mod.json` を読む。

> 注: このディレクトリは [Manual リポジトリ](https://github.com/Codeer-Software/Codeer.LowCode.Blazor.Manual) の `JP/patterns/` からの同期コピー。**編集は Manual 側で行い、`ClaudeTmp/sync_patterns_to_designer_docs.py` で再同期** する。画像参照はコメントアウト済み、Manual 内の他セクションへのリンクは絶対URL化されている。
