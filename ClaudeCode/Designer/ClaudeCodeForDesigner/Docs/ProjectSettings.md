...
> **注意 (Claude 向け)**: このファイルは接続文字列など秘密情報の置き場で、**中身を読む必要があるデザイン作業は無い**
> (DB のスキーマ・データ確認は `sql` / `designcheck` CLI が接続文字列を内部で解決する)。
> 読む/書くのはユーザーが明示的に依頼したときだけ (ワークスペースの ask 設定で確認が出る)。
> その場合も、許可を求める前に「内容 (接続文字列やパスワード) は LLM へ送信され会話ログにも残る」ことを一言伝え、
> リスクを了解のうえで承認してもらうこと。
> データソースやデプロイ設定の追加はデザイナの右クリックメニューからもできる。
