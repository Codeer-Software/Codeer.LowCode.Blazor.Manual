...
> **`bool` への変換も同じルール**: 数値（`0`=false、非0=true）や文字列 `"True"` / `"False"`（大文字小文字は問わないので `"true"` も可）は `bool` に変換されるが、**それ以外の文字列を `bool` に入れると実行時エラー**になる。代入・初期化だけでなく**戻り値**でも同じで、`bool Foo() { return "true"; }` は動くが `bool Foo() { return "xxx"; }` は呼び出し時に実行時エラーになる（**どちらも designcheck は通る**ので、文字列を `bool` に渡すコードは値が必ず `"True"`/`"False"` になることを書き手が保証する）。
