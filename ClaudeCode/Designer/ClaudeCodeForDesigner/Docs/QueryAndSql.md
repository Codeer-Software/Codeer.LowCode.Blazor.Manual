...
ソースコード `Source/Codeer.LowCode.Blazor/Repository/Design/{QuerySetting,ExecuteSqlSetting}.cs` から転記。`Parameters` は `List<DbParameterSetting>` (= JSON 配列)。`[Obsolete]` プロパティ (`QueryText` / `CountSqlText` / `SqlText` 等) は実体ファイル側の `.sql` ファイルから読み込まれるバッファなので JSON に書かない。

```csharp
public enum QuerySortType   { None, System }
public enum QueryPagingType { None, System, CustomCountQuery }

public class QuerySetting
{
    [Obsolete] public string QueryText { get; set; } = string.Empty;
    [Obsolete] public string CountSqlText { get; set; } = string.Empty;
    [Obsolete] public bool UseCustomPaging { get; set; }
    [Obsolete] public SortAndPagingType SortAndPagingType { get; set; }
    public QuerySortType QuerySortType { get; set; }
    public QueryPagingType QueryPagingType { get; set; }
    public List<DbParameterSetting> Parameters { get; set; } = new();   // 配列
}

public enum ExecuteSqlCommandType { Sql, StoredProcedure }
public enum ExecuteSqlMethodType  { NonQuery, Reader, Scalar }

public class ExecuteSqlSetting
{
    public ExecuteSqlCommandType CommandType { get; set; }
    public ExecuteSqlMethodType MethodType { get; set; }
    public NewId? NewId { get; set; }                                    // INSERT 時の新 ID 取得
    public FailedCondition? FailedCondition { get; set; } = new();
    public List<DbParameterSetting> Parameters { get; set; } = new();    // 配列
    [Obsolete] public string SqlText { get; set; } = string.Empty;
}

public class NewId
{
    public bool IsMethodReturn { get; set; }                             // メソッド戻り値で取得
    public string ParameterName { get; set; } = string.Empty;            // 出力パラメータで取得
}

public enum FailedConditionCompareType { Equal, NotEqual }

public class FailedCondition
{
    public bool IsMethodReturn { get; set; }
    public string ParameterName { get; set; } = string.Empty;
    public FailedConditionCompareType CompareType { get; set; }
    public string CompareValue { get; set; } = string.Empty;
}

public enum DbParameterDirection { Input, Output, InputOutput, ReturnValue, MethodReturn }

public class DbParameterSetting
{
    public const string ROWS_PER_PAGE_PARA = "rows_per_page";   // システム予約 (明示宣言不要)
    public const string OFFSET = "offset";                       // システム予約 (明示宣言不要)
    public bool IsParameter { get; set; }   // true=入力パラメータ (SQL の @name), false=出力列定義
    public string Name { get; set; } = string.Empty;
    public string DbType { get; set; } = string.Empty;           // "bigint" / "varchar" / "integer" / "DATE" 等
    public DbParameterDirection DbParameterDirection { get; set; }
}
```

> **注意 (Claude 向け)**: `Parameters` は `List<DbParameterSetting>` 配列。SQL は別ファイル (`{Module}.{FieldName}.sql`) から読み込まれるので JSON に SQL 文字列を書かない。`rows_per_page` / `offset` はシステム予約名で `Parameters` に明示宣言不要。
