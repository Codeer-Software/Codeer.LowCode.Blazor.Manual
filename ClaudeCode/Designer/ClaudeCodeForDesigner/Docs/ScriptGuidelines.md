...
`CreatedAt` / `UpdatedAt` / `Creator` / `Updater` は CLB が自動でセットする項目なので、**編集フォームに入力欄として置かない**。出すなら読み取り専用表示（DetailLayout の列に `"IsViewOnly": true`）か、一覧 (List) の列のみにする。

`IsUpdateProtected: true` を付けても**更新時に上書きしないだけ**で、新規作成時の手入力も UI 上の編集も防げない。「自動で入る項目をユーザーが触れてしまう」矛盾を避けるには `IsViewOnly` で表示専用にすること。読み取り専用の作り方は [LayoutGuidelines.md](LayoutGuidelines.md) の「読み取り専用・計算結果フィールドの正攻法」を参照。
