...
## 52. 検索条件の Select→Select 連動は OnSearchDataChanged で SearchValue → Value をコピーする

**症状:** SelectField の `SearchCondition` + `FieldVariableMatchCondition` で「親 Select の値で候補を絞る」を宣言した連動 (CascadeInputBySearch パターン) を、そのまま**検索レイアウト**に置くと連動しない。親を選んでも子の候補が空のまま (または絞られない)。

**原因:** 連動の候補絞り込み (`FieldVariableMatchCondition` の `Variable`) が参照するのは data の **`Value`**。しかし検索フォームの入力は **`SearchValue`** に入り、`Value` は空のまま。さらに `SearchValue` の変更は他フィールドへの変更通知 (候補の取り直し) を発火しない。

**対策:** 親 Select の `OnSearchDataChanged` で 1 行コピーする。

```csharp
// 親 Select (例: SelectedProject) の OnSearchDataChanged に指定
...
Value が変わると CLB が連動先の候補をリアルタイムに取り直し、**候補から外れた子の SearchValue も自動でクリア**される (枠組みが全部やってくれる)。
