...
> **LinkField との使い分け（候補が少ないマスタは Select）**: 「マスタテーブルで管理する」＝ [LinkField](LinkField.md) ではない。費目・区分・ステータスのように**候補が数件〜十数件で、ドロップダウンから直接選べる**ものは、検索ピッカーの LinkField ではなく **SelectField（マスタ参照）** を使う方が速くて分かりやすい。LinkField は「候補が多く、検索して 1 件選ぶ」前提。マスタ管理とドロップダウン選択は両立する。
>
> **SelectField でマスタ参照にする書き方**: `Candidates: []`（空）にして、`SearchCondition.ModuleName` = マスタのモジュール名、`SearchCondition.LimitCount: 100` 程度、`ValueVariable: "Id.Value"`、`DisplayTextVariable: "Name.Value"`、`DbColumn` = 外部キー列。正典サンプル: `Samples/PatternShowcase/App/Modules/CascadeSearch.mod.json` の `SelectedProject`（Project マスタ参照）。
