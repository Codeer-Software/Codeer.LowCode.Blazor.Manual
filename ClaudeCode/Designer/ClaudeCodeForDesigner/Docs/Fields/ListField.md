...
> **いつ使う**: 子レコードを**列の揃った表**で見せるならこれ（均一な明細表の第一候補）。1 レコードが複雑なフォームなら [DetailListField](DetailListField.md)、カード/タイル並べなら [TileListField](TileListField.md)。選定ガイドは [DetailListField.md](DetailListField.md) の「どちらを選ぶか」を参照。
