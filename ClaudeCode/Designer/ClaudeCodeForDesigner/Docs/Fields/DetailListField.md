...
インライン編集可能な子レコードの一覧。ListField がテーブル形式（ListLayout）で表示するのに対し、DetailListField は各行を DetailLayout（フォーム形式）で表示する。**1 レコードを 1 枚のフォーム/カードとして並べたいとき**に使う。

> ⚠ **「明細表 = DetailListField」は誤り (最頻出の致命傷)。** 名前に「明細 (Detail)」が入っているが、業務の「明細行」を表すフィールドではない。"Detail" は「各行を **DetailLayout (フォーム)** で描く」という意味。**注文＋明細・請求＋明細・経費精算＋明細のような、列の揃った明細表は [`ListField`](ListField.md) を使う**（正典 `Samples/PatternShowcase/App/Modules/Order.mod.json` の `Details` も `ListFieldDesign`）。**迷ったら `ListField`。** → [../CommonMistakes.md](../CommonMistakes.md) の #53、下の「どちらを選ぶか」。
...
PatternShowcase の DetailListField/TileListField 子モジュール (例: `DetailListDemoChild`, `LookupCustomer`) は全てカード化済み。

## JSON例

### 子レコードをフォーム形式でインライン編集する例

> ⚠ この例は **`DetailListField` の使い方**を示すもの。**列の揃った明細表（注文明細など）は `ListField` が正解**で、これではない（[ListField.md](ListField.md) / [../CommonMistakes.md](../CommonMistakes.md) #53）。下記は「各行を DetailLayout のフォームで編集したい」と明確に判断したときの形。
