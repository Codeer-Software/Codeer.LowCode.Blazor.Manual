...
> **いつ使う**: 各レコードを**カード/タイル**としてグリッド状に並べたいとき（カタログ・ギャラリー等）。列の揃った表なら [ListField](ListField.md)、複雑なフォームのインライン編集なら [DetailListField](DetailListField.md)。選定ガイドは [DetailListField.md](DetailListField.md) の「どちらを選ぶか」を参照。
