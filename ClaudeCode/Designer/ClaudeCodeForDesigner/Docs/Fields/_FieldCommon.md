...
public abstract class ListFieldDesignBase : FieldDesignBase, IListFieldDesign, IDisplayName
{
    ...
    public ListReplaceMode ReplaceMode { get; set; }   // enum: None / All / UpdateAsDeleteInsert (洗い替え)
...
| `ReplaceMode` | ListReplaceMode | `"None"` | 洗い替え。`All` = Submit 時に SearchCondition 一致分を全件削除して現在行を全件新規追加 / `UpdateAsDeleteInsert` = 変更行だけ削除+新規追加に置き換え (一意キー入れ替え対応)。詳細は [ListField.md](ListField.md) の洗い替えセクション参照。 |
...
**注意: 検索フォームの入力値は `Value` ではなく `SearchValue` に入る。** 候補絞り込み (`SearchCondition` の `FieldVariableMatchCondition`) が参照するのは `Value` のため、検索条件の Select 連動では `SearchValue` を `Value` にコピーする。

```csharp
// mod.json: { "OnSearchDataChanged": "SelectedProject_OnSearchDataChanged", "Name": "SelectedProject" }
void SelectedProject_OnSearchDataChanged()
{
    // 検索条件の Select 連動: SearchValue を Value にコピーすると
    // 連動先 (FieldVariableMatchCondition で SelectedProject.Value を参照する Select) の
    // 候補がリアルタイムに取り直され、候補から外れた選択値は自動クリアされる
