...
namespace LowCodeApp.WinForms.Designer.Lib
{
    internal static class DbMapping
    {
        ...
        private static List<string> CreateColumnDefinition(DataSourceType dataSourceType, FieldDesignBase field)
        {
            ...
            else if (field is PasswordHashFieldDesign passwordHash)
            {
                var columnType = dataSourceType switch
                {
                    DataSourceType.SQLite => "TEXT",
                    DataSourceType.SQLServer => "NVARCHAR(MAX)",
                    DataSourceType.PostgreSQL => "TEXT",
                    DataSourceType.Oracle => "VARCHAR2(4000)",
                    _ => throw new Exception()
                };

                var list = new List<string>();
                if (!string.IsNullOrEmpty(passwordHash.DbColumnHash)) list.Add($"  {passwordHash.DbColumnHash} {columnType}");
                if (!string.IsNullOrEmpty(passwordHash.DbColumnSalt)) list.Add($"  {passwordHash.DbColumnSalt} {columnType}");
                return list;
            }
