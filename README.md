...
そのままでも業務で使える完成形のアプリをテンプレート化したもの。デザイナで新規プロジェクト作成時にテンプレートを選ぶだけで、画面・データモデル・サンプルデータが一式展開されます。自社向けに改造して本番運用へ持っていく起点として使ってください。

- [業務テンプレート一覧](JP/templates/templates.md) — 3 つのテンプレートの位置づけと共通の使い始め方
- [在庫管理テンプレート](JP/templates/inventory_management.md) — 入出庫・棚卸し・発注・要発注品アラート・在庫ダッシュボード
- [営業支援 (SFA) テンプレート](JP/templates/sfa.md) — 商談・活動履歴・顧客管理・パイプラインダッシュボード
- [プロジェクト管理テンプレート](JP/templates/project_management.md) — プロジェクト・タスク・ガント・タスクボード・進捗ダッシュボード
...
- [レスポンシブ対応の実現方法](JP/look_and_feel/responsive.md) — 画面幅・デバイスへの追従に使う機能のまとめ
...
本リポジトリに掲載するすべての情報・成果物（ドキュメント、サンプルコード、サンプルアプリケーション、テンプレート、設定例、画像等）は現状有姿 (AS IS) で提供され、内容の正確性・完全性・最新性・特定目的への適合性を含め、いかなる保証もありません。記載内容は製品のバージョンアップ等により予告なく変更されることがあります。

これらを参考・利用・流用したこと（サンプルやテンプレートを元に構築したアプリケーションの動作・運用を含む）に起因するいかなる損害についても、Codeer は一切の責任を負いません。利用はすべて利用者自身の責任と判断で行ってください。
