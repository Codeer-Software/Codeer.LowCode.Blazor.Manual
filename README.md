...
- [アプリ作成パターン入口](JP/patterns/patterns.md) — PatternShowcase テンプレートを起点にしたパターンのインデックス。単一CRUD / ヘッダ詳細 / マスタ参照 / 多対多 / ツリー / 双方向ID持ち合い / 状態遷移 / 履歴 / 論理削除 / システムフィールドの各パターンに「アプリ → データ構造 → モジュール構成」の流れで解説
