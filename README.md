...
- [アプリ作成パターン入口](JP/patterns/patterns.md) — PatternShowcase テンプレートを起点にしたパターンのインデックス。単一CRUD / ヘッダ詳細 / マスタ参照 / 多対多 / 階層構造 / 双方向ID持ち合い / 論理削除 / システムフィールドの各パターンに「アプリ → データ構造 → モジュール構成」の流れで解説
