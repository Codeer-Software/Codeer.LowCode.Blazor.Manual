...
- [アプリ作成パターン入口](JP/patterns/patterns.md) — PatternShowcase テンプレートを起点にしたパターンのインデックス。単一CRUD / ヘッダ詳細 / 多段ネスト / 多対多 / 階層構造 / 一括処理 / 取込書出 / 楽観ロック / 論理削除 / 自動保存 / システムフィールド / マスタ参照 / 双方向ID持ち合いの 13 パターンに「アプリ → データ構造 → モジュール構成」の流れで解説
