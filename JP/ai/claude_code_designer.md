...
### 5. プロジェクト固有の情報も `CLAUDE.md` に蓄積する

`CLAUDE.md` は **そのプロジェクト固有の前提・規約**を書いておく場所として活用してください。Claude Code と作業する中で気づいた都度、口頭で「これも `./CLAUDE.md` に追記して」と伝えると蓄積できます。
...
- **リファレンスを最新に保つ**: フレームワーク／マニュアルが更新されたら ZIP を再ダウンロードし、`CLAUDE.md` と `ClaudeCodeForDesigner/` を上書きしてください（プロジェクト固有の追記はマージするか退避してから適用）。
