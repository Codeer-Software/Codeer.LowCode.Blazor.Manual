...
- サイドバーに **データ操作 / リスト / 一覧 / 検索 / レイアウト / 入力UX / 出力 / ダイアログ等 / 別フレーム / 認証・権限 / 承認** のカテゴリで各パターンへのリンクが並んでいる
- ログイン付き。admin / admin（管理者）と alice / bob / carol / dave（一般ユーザー、パスワード: test）が登録済み
...
| [**双方向 ID 持ち合い (1:1)**](bidirectional.md) | 会社↔代表者、注文↔領収書 |
...
### J. [認証・権限・承認のパターン](auth_patterns.md)

ログインがあって初めて成り立つパターン。標準パターン集のサイドバー「認証・権限」「承認」グループに収録 (4 個別ファイル):
...
- [一覧ページパターン集](https://github.com/Codeer-Software/Codeer.LowCode.Blazor.Designer.Standard/blob/main/Source/Codeer.LowCode.Blazor.Designer.Standard/ClaudeWorkspace/ClaudeCodeForDesigner/Docs/ListPagePatterns.md) — 一覧ページ系の追加パターン
