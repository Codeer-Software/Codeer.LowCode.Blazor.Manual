...
「消したデータは残したいが、元テーブルは肥大化させたくない」場合は、行を残す論理削除ではなく、**削除時に別テーブル（退避テーブル）へコピーしてから元テーブルから物理削除する**方式が使える。`DeleteArchiveField` を Module に 1 つ置くだけで削除操作がこの動作に切り替わる。
...
- `DeletedAt` は **DateTime**、`Deleter` は **Link (→ AppUser)** で作る。型が違うと削除マーカーとして認識されない
