...
- 一般ユーザー (alice 等): サイドバーの「認証・権限」グループに「マイプロフィール / 個人メモ / 自分宛タスク / 削除監査 / 管理画面へ」、「承認」グループに「経費精算 / 休暇申請 / 承認待ち / 承認状況」が表示
- 「管理画面へ」リンクをクリック → 管理画面 PageFrame (`AdminFrame`) に遷移
- 管理画面のサイドバーは「ユーザー管理 / 削除監査 (管理) / 承認経路マスタ / メインへ戻る」
...
|---|---|---|
| `Main` | 一般ユーザー向け | マイプロフィール / 個人メモ / 自分宛タスク / 経費精算 / 休暇申請 / 承認待ち / **管理画面へ** |
| `AdminFrame` | 管理者向け (UserReadCondition で制限) | ユーザー管理 / 削除監査 (管理) / 承認経路マスタ / **メインへ戻る** |
...
- サイドバー **`認証・権限/管理画面へ (admin 限定)`** → `Main` → `AdminFrame` への切替リンク
- 管理画面サイドバー **`ユーザー管理`** / **`削除監査 (管理)`** / **`承認経路マスタ`** → `AdminFrame` 配下のリンク
