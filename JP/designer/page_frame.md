...
1. **`アプリケーションルート` ON** で、`トップページ` にモジュール指定があり、**ユーザーアクセス権にマッチ**する PageFrame のうち、**`優先度` が最も大きいもの**（同じ値なら定義順で先頭）
2. 上記がなければ、`トップページ` にモジュール指定のある **最初の PageFrame**

> ⚠ **ハマりどころ**: 複数 PageFrame がある場合、ルートで開きたいものに `アプリケーションルート` を設定してください。1 人のユーザーが複数のアプリケーションルートにアクセスできる構成（例: 管理者用と一般用の両方に入れる人）では、**`優先度` が大きい PageFrame が優先して開きます**。`優先度` が同じ（初期値 0 のまま）だと「どれが開くかは定義順に依存」します。
...
| **優先度** (Priority) | 複数のアプリケーションルートにアクセスできるユーザーで、どれを先に開くか。**数値が大きいほど優先**（初期値 0）。`アプリケーションルート` が ON のときだけ意味を持つ |
| **対象デバイス** (TargetDevice) | ルート URL で開くときにこの PageFrame を採用するデバイス。`Any`（既定・デバイス問わず）/ `PC`（マウス等の細かいポインタ）/ `Touch`（タッチ端末）。`アプリケーションルート` が ON のときだけ意味を持つ |
| **適用開始幅** (WidthFrom) | ルート URL で開くときにこの PageFrame を採用する**画面幅の下限**（px）。「画面幅がこの値以上のとき対象」。未指定なら幅の条件なし。`アプリケーションルート` が ON のときだけ意味を持つ |

### 画面幅・デバイスでアプリケーションルートを切り替える

複数のアプリケーションルートに **対象デバイス** / **適用開始幅** を設定すると、ルート URL（`/`）を開いたときに使う PageFrame を画面サイズやデバイスで出し分けできます（PC は通常のサイドバー型、スマホは縦長のコンパクト型、のような構成）。

選択ルール（ルート URL を開いた瞬間に判定）:

1. アクセス可能なアプリケーションルートのうち、対象デバイス・適用開始幅の条件に一致するものが候補になる
2. デバイス指定（`PC` / `Touch`）のあるものが `Any` より優先
3. **適用開始幅が一番大きい**（= 一番フィットする）ものが選ばれる
4. 同値なら **優先度**（大きいほど優先）

補足:

- どの PageFrame にも対象デバイス・適用開始幅が設定されていない場合は、従来どおり優先度だけで選ばれます（後方互換）
- URL でモジュールを直接指定した場合（`/Main/Home` 等）はこの判定の対象外です
- 例: `Main`（適用開始幅 900）と `Compact`（条件なし）の 2 つを用意すると、画面幅 900px 以上では Main、900px 未満では Compact が開きます

動くサンプルは標準パターン集の **`別フレーム/画面幅で切替`** を参照してください。

### 自動ズーム（画面幅に合わせて全体を縮小・拡大）

PC 向けに固定幅で作った画面を、レイアウトを作り直さずに**そのまま縮めて/拡げて**別の画面幅に収める機能です。ブラウザのズーム（Ctrl + マウスホイール）と同じように、フォント・余白・間隔まで含めて **UI 全体を一律の倍率で拡大縮小**します。折り返しや列の再配置は行いません。

| 日本語表示名 | 内容 |
|---|---|
| **自動ズーム** (AutoZoom) | `なし`（既定・ズームしない）/ `FitToWidth`（画面幅に合わせて全体を拡大縮小） |
| **基準幅** (BaseWidth) | この画面を「設計したときの幅」（px）。実際の画面幅がこの値と違うと、その比率で全体がズームされる。`自動ズーム` を `FitToWidth` にしたときだけ意味を持つ |

動作:

- 実際のコンテンツ幅 ÷ **基準幅** の比率で全体をズームします。基準幅より狭ければ縮小、広ければ拡大します
- **基準幅はコンテンツ部分（メイン領域）の幅**です。サイドバーを表示している場合、その表示中のサイドバー幅は内部で自動的に加算してから比率を計算するので、基準幅にはサイドバーを除いたコンテンツ幅を指定します
- **基準幅を指定しない、または `自動ズーム` が `なし` の場合はズームしません**

使いどころ:

- PC 用に作り込んだ固定幅の画面を、タブレット等でも崩さずそのまま見せたいとき。作り直しのコストをかけずに収める割り切りとして手早いです

注意:

- 全体を一律に縮める機能なので、**折り返し系（`IsWrap` / `IsAutoFillWrap`）や比率伸縮（`IsProportionalScale`）とは役割が重なります**。1 つの PageFrame では、自動ズームで逃がすか、折り返し系で追従させるか、どちらかに寄せるのがおすすめです
- 画面幅ごとに**構成そのものを変えたい**場合は、自動ズームではなく [画面幅・デバイスでアプリケーションルートを切り替える](#画面幅デバイスでアプリケーションルートを切り替える)（PageFrame を分けて出し分け）を使います。自動ズームは「同じ画面をそのまま拡大縮小」、フレーム切替は「別の画面に差し替え」です

動くサンプルは標準パターン集の **`別フレーム/自動ズーム`**（`ZoomFrame`）を参照してください。
...
| 症状 | 原因として多いもの |
|---|---|
| ルート URL で意図しない PageFrame が開く | `アプリケーションルート` が未設定、または複数のルートにアクセスでき `優先度` が同値（定義順依存）。先に開きたい方の `優先度` を大きくする |
