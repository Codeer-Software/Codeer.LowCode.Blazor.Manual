...
> 注意：DBのデータを追加・更新・削除するのに、一意性をもつ`Idフィールド`がモジュールに存在する必要があります。　`Idフィールド`はシステムフィールドであるため、名前の変更はできません。
...
一覧画面で段と列の数量を設定したうえで、`ツールボックス`の`未使用フィールド`からDBフィールドをドラッグ・ドロップで一覧画面に追加します。
