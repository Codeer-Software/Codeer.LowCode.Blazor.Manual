...
| **`false`（詳細検索）** | 比較演算子・空白／空白でないモードをユーザーに選ばせる | 入力欄 + 比較ドロップダウン |
...
Field の `AllowEmptySearch=true` で、詳細検索（`IsSimpleSearchParameter=false`）のモードドロップダウンに **「空白」「空白でない」** が追加されます。
...
| Field | 既定の演算子（簡易） | 詳細時に選べるもの |
|---|---|---|
| [TextField](Text.md#検索での挙動) | 部分一致 | 部分一致 / 一致 / 空白 / 空白でない |
| [NumberField](Number.md#検索での挙動) | ≥（以上） | 範囲 / 空白 / 空白でない |
| [DateField](Date.md#検索での挙動) | ≥（以降） | 範囲 / 空白 / 空白でない |
| [DateTimeField](DateTime.md#検索での挙動) | ≥（以降） | 範囲 / 空白 / 空白でない |
| [TimeField](Time.md#検索での挙動) | ≥（以降） | 範囲 / 空白 / 空白でない |
| [BooleanField](Boolean.md#検索での挙動) | True/False ドロップダウン | 上記 + 空白 / 空白でない |
| [SelectField](Select.md#検索での挙動) | 候補ドロップダウン（一致） | 一致 + Or 検索（複数選択） |
| [RadioGroupField](RadioGroup.md#検索での挙動) | 候補ラジオ（一致） | 同上 |
| [LinkField](Link.md#検索での挙動) | 検索ダイアログ → ID 一致 | 一致 / 空白 / 空白でない |
| [IdField](Id.md#検索での挙動) | 部分一致 | 部分一致 / 一致 |
