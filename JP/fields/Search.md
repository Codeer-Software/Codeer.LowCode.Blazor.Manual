...
|---|---|---|
| [TextField](Text.md#検索での挙動) | 部分一致 | 部分一致 / 完全一致 / 空 / 空以外 |
| [NumberField](Number.md#検索での挙動) | ≥（以上） | 範囲 / 空 / 空以外 |
| [DateField](Date.md#検索での挙動) | ≥（以降） | 範囲 / 空 / 空以外 |
| [DateTimeField](DateTime.md#検索での挙動) | ≥（以降） | 範囲 / 空 / 空以外 |
| [TimeField](Time.md#検索での挙動) | ≥（以降） | 範囲 / 空 / 空以外 |
| [BooleanField](Boolean.md#検索での挙動) | True/False ドロップダウン | 上記 + 空 / 空以外 |
| [SelectField](Select.md#検索での挙動) | 候補ドロップダウン（完全一致） | 完全一致 + Or 検索（複数選択） |
| [RadioGroupField](RadioGroup.md#検索での挙動) | 候補ラジオ（完全一致） | 同上 |
| [LinkField](Link.md#検索での挙動) | 検索ダイアログ → ID 完全一致 | 完全一致 / 空 / 空以外 |
| [IdField](Id.md#検索での挙動) | 部分一致 | 部分一致 / 完全一致 |
