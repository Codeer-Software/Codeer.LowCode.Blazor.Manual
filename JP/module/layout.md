...
Row の **「列幅を比率で拡大縮小」**（`IsProportionalScale: true`）を有効にすると、その行の各列の `Width` を比率として扱い、画面幅が変わっても列幅の比率を保ったまま行幅いっぱいに拡大縮小します。例えば `Width` を `100 / 200 / 100` と指定すると、実際の幅は常に行幅の 25% / 50% / 25% になります。

制約（デザインチェックで検証されます）:

- 行内の**すべての列に `Width` の指定が必須**
