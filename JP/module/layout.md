...
Rowは下部に適用されるマージン `1rem` が設定されています。ただし、次の条件が満たされる時に適用されるマージンが変化します。
- `Row` のプロパティ、 `IsRowMarginRemoved` が `true` に設定された場合は、マージンが適用されません。
- `Row` に含まれる直下の `Column` について枠線が設定されている場合上部にマージン `1rem` が設定されます。
